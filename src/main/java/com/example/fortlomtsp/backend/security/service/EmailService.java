...
public class EmailService {
    ...
    @Autowired
    UserAccountService personaService;
    ...
    public ResponseEntity<?> sendEmailTemplate(EmailValues dto){
        Optional<UserAccount> usuarioOpt = personaService.getbyNombreUsuarioOrEmail(dto.getMailTo());
        if(!usuarioOpt.isPresent())
            return new ResponseEntity(new Message("No existe ningún usuario con esas credenciales"), HttpStatus.NOT_FOUND);
        UserAccount persona = usuarioOpt.get();
    ...
    public void sendEmail(EmailValues dto) {
     ...
     public ResponseEntity<?> changePassword(ChangePassword dto, BindingResult bindingResult){
        ...
        Optional<UserAccount> usuarioOpt = personaService.getByTokenPassword(dto.getTokenPassword());
        if(!usuarioOpt.isPresent())
            return new ResponseEntity(new Message("No existe ningún usuario con esas credenciales"), HttpStatus.NOT_FOUND);
        UserAccount usuario = usuarioOpt.get();
